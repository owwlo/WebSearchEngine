 ...
 * change the interface of this class. In HW1: instructor's
 * {@link IndexerFullScan} is provided as is. In HW2: students will implement
 * {@link IndexerInvertedDoconly}, {@link IndexerInvertedOccurrence}, and
 * {@link IndexerInvertedCompressed}. See comments below for more info. In HW3:
 * students will incorporate the corpus analysis and log mining results into
 * indexing process through {@link CorpusAnalyzer} and {@link LogMiner}.
 * 
 * @author congyu
 * @author fdiaz
 */
public abstract class Indexer {
     ...
     * certain inverted list. The index must reside at the directory of
     * index_prefix, no other data can be stored (either in a hidden file or in
     * a temporary directory). In serve mode, the constructed index should
     * provide the necessary functionality to support the search tasks. In HW3:
     * leverage load() functions from both {@link CorpusAnalyzer} and
     * {@link LogMiner}.
     */
     ...
     * serving the search traffic. You must load the index from the constructed
     * index above, do NOT try to reconstruct the index from the corpus. When
     * the search engine is run in serve mode, it will NOT have access to the
     * corpus, all grading for serve mode will be done with the corpus removed
     * from the machine.
     */
    public abstract void loadIndex() throws IOException, ClassNotFoundException;

    /**
     * APIs for statistics needed for ranking. {@link numDocs} and
     * {@link totalTermFrequency} must return correct results for the current
     * state whenever they are called, either during index construction or
     * during serving (obviously). {@link corpusDocFrequencyByTerm},
     * {@link corpusTermFrequency}, and {@link documentTermFrequency} must
     * return correct results during serving.
     */
    ...
    public abstract Map<String, Integer> documentTermFrequencyMap(int docid);
